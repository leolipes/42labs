...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/22 18:26:12 by lfilipe-          #+#    #+#             */
/*   Updated: 2022/02/24 12:00:01 by lfilipe-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void *ff_open(const char *path, int flags)
{
	...
	if (flags & MG_FS_WRITE)
		mode |= FA_WRITE | FA_CREATE_ALWAYS | FA_OPEN_APPEND;
...
static int getrange(struct mg_str *s, int64_t *a, int64_t *b)
{
	...
	for (i = 0; i + 6 < s->len; i++)
	{
		if (memcmp(&s->ptr[i], "bytes=", 6) == 0)
		{
			...
			if (p.len > 0 && p.ptr[0] >= '0' && p.ptr[0] <= '9')
				numparsed++;
			*a = mg_to64(p);
			// MG_INFO(("PPP [%.*s] %d", (int) p.len, p.ptr, numparsed));
			while (p.len && p.ptr[0] >= '0' && p.ptr[0] <= '9')
				p.ptr++, p.len--;
			if (p.len && p.ptr[0] == '-')
				p.ptr++, p.len--;
			*b = mg_to64(p);
...
void mg_http_delete_chunk(struct mg_connection *c, struct mg_http_message *hm)
{
	...
	ce = &ch.ptr[ch.len];
	if (ce < end)
		memmove((void *)ch.ptr, ce, (size_t)(end - ce));
	c->recv.len -= ch.len;
	if (c->pfn_data != NULL)
		c->pfn_data = (char *)c->pfn_data - ch.len;
}
...
bool mg_log_prefix(int level, const char *file, int line, const char *fname)
{
	...
	while (mg_commalist(&s, &k, &v))
	{
		if (v.len == 0)
			max = atoi(k.ptr);
		if (v.len > 0 && strncmp(p, k.ptr, k.len) == 0)
			max = atoi(v.ptr);
	}
...
void mg_mqtt_login(struct mg_connection *c, struct mg_mqtt_opts *opts)
{
	...
	if (opts->clean || cid.len == 0)
		connflag |= MQTT_CLEAN_SESSION;
	if (opts->will_retain)
		connflag |= MQTT_WILL_RETAIN;
...
static long mg_sock_send(struct mg_connection *c, const void *buf, size_t len)
{
	...
	return n == 0 ? -1 : n < 0 && mg_sock_would_block() ? 0
														: n;
}
...
static long mg_sock_recv(struct mg_connection *c, void *buf, size_t len)
{
	...
	if (c->is_udp)
	{
		...
		if (n > 0)
			tomgaddr(&usa, &c->peer, slen != sizeof(usa.sin));
	}
	else
	{
...
static void connect_conn(struct mg_connection *c)
{
	...
	if (getsockopt(FD(c), SOL_SOCKET, SO_ERROR, (char *)&rc, &len))
		rc = 1;
#endif
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms)
{
	...
	for (c = mgr->conns; c != NULL; c = tmp)
	{
		...
		else
		{
			...
			if (c->is_writable)
				write_conn(c);
			while (c->is_tls && read_conn(c) > 0)
				(void)0; // Read buffered TLS data
		}
...
static char *mg_ssi(const char *path, const char *root, int depth)
{
	...
	if (fp != NULL)
	{
		...
		if (len > 0)
			mg_iobuf_add(&b, b.len, buf, len, align);
		if (b.len > 0)
			mg_iobuf_add(&b, b.len, "", 1, align); // nul-terminate
		fclose(fp);
	}
	(void)depth;
	(void)root;
...
{
	size_t kvlen, kl;
	for (kvlen = 0; ofs + kvlen < n && s[ofs + kvlen] != ',';)
		kvlen++;
...
long mg_tls_recv(struct mg_connection *c, void *buf, size_t len)
{
	...
	return n == 0 ? -1 : n == MBEDTLS_ERR_SSL_WANT_READ ? 0
														: n;
}
...
long mg_tls_send(struct mg_connection *c, const void *buf, size_t len)
{
	...
	return n == 0 ? -1 : n == MBEDTLS_ERR_SSL_WANT_WRITE ? 0
														 : n;
}
#endif
...
void mg_tls_init(struct mg_connection *c, struct mg_tls_opts *opts)
{
	...
	if (opts->ciphers != NULL)
		SSL_set_cipher_list(tls->ssl, opts->ciphers);
	if (opts->srvname.len > 0)
	{
		char mem[128], *buf = mem;
		mg_asprintf(&buf, sizeof(mem), "%.*s", (int)opts->srvname.len,
...
long mg_tls_recv(struct mg_connection *c, void *buf, size_t len)
{
	...
	return n == 0 ? -1 : n < 0 && mg_tls_err(tls, n) == 0 ? 0
														  : n;
}
...
int mg_check_ip_acl(struct mg_str acl, uint32_t remote_ip)
{
	...
	while (mg_commalist(&acl, &k, &v))
	{
		...
		if (parse_net(&k.ptr[1], &net, &mask) == 0)
			return -2;
		if ((mg_ntohl(remote_ip) & mask) == net)
			allowed = k.ptr[0];
	}
	return allowed == '+';
}
